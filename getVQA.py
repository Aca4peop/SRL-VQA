...
from database.VQA.dataset_yuv import VideoDataset, DisTypeVideoDataset
...
def getLIVEVQA(cfg, transform_train, transofrm_test, batch_train=1, batch_test=1, train_percent=0.8, idx=-1):
    ...
    if cfg.IDX != -1:
        subj_dataset = './database/VQA/LIVE/LIVE_LEAVE2_' + str(cfg.IDX) + '_LAST.json'
        if cfg.IDX == 100:
...
def getCSIQVQA(cfg, transform_train, transofrm_test, batch_train=1, batch_test=1, train_percent=0.8, idx=-1):
    ...
    if cfg.IDX != -1:
        subj_dataset = './database/VQA/CSIQ/CSIQ_LEAVE2_' + str(cfg.IDX) + '_LAST.json'
        if cfg.IDX == 100:
...
def getVQCVQA(cfg, transform_train, transofrm_test, batch_train=1, batch_test=1, train_percent=0.8, idx=-1):
    ...
    if cfg.IDX != -1:
        subj_dataset = './database/VQA/LIVE-VQC/VQC_subj_score_' + str(cfg.IDX) + '.json'
        if cfg.IDX == 100:
            subj_dataset = './database/VQA/LIVE-VQC/VQC_subj_score_TEST.json'
            video_dataset = {x: VideoDataset(subj_dataset, video_path, x, channel, size_x, size_y, stride_x, stride_y, frameWant=cfg.VQA.FRAMEWANT, transform=transform_train) for x in ['test']}
            dataloaders = {x: DataLoader(video_dataset[x], batch_size=batch[x], shuffle=True, num_workers=cfg.DATA.NUM_WORKERS , drop_last=False) for x in ['test']}
            return writer, check_path, None, dataloaders['test'], logger
    ...
    video_dataset = {x: VideoDataset(subj_dataset, video_path, x, channel, size_x, size_y, stride_x, stride_y, frameWant=cfg.VQA.FRAMEWANT, transform=transform_train) for x in ['train', 'test']}
    dataloaders = {x: DataLoader(video_dataset[x], batch_size=batch[x], shuffle=True, num_workers=cfg.DATA.NUM_WORKERS , drop_last=False) for x in ['train', 'test']}
    return writer, check_path, dataloaders['train'], dataloaders['test'], logger
...
def getKonVQA(cfg, transform_train, transofrm_test, batch_train=1, batch_test=1, train_percent=0.8, idx=-1):
    ...
    if cfg.IDX != -1:
        subj_dataset = './database/VQA/konIQ/KON_hzy_' + str(cfg.IDX) + '.json'
        # subj_dataset = './database/VQA/konIQ/KON_hzy_' + str(cfg.IDX) + '_mp4.json'
        if cfg.IDX == 100:
