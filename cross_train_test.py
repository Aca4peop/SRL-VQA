...
from database.VQA.dataset_yuv import DisTypeVideoDataset, VideoDataset
...
def getCSIQVQA(cfg, transform_train, transofrm_test, batch_train=1, batch_test=1, train_percent=0.8, idx=-1, train=True):
    ...
    video_path = '/home1/server823-2/database/2D-Video/CSIQVideo'
    batch = {'train': batch_train, 'test': batch_test}
...
def getKonVQA(cfg, transform_train, transofrm_test, batch_train=1, batch_test=1, train_percent=0.8, idx=-1, train=True):
    ...
    video_path = '/home2/wzw/LCVQA/kon_yuv'
...
def getCVDVQA(cfg, transform_train, transofrm_test, batch_train=1, batch_test=1, train_percent=0.8):
    summary_dir = 'runs/swin_CVD_VQA/' + DAY
    writer = SummaryWriter(summary_dir)
    check_path = './checkpoints/' + 'CVD_VQA/'
    logger = getlogger(path='CVD_VQA')
    ...
    # TODO: 没修改subj_dataset
    subj_dataset = './database/VQA/CVD2014/cvd_subj_score.json'
    video_path = '/home2/wzw/LCVQA/cvd_yuv' 
    batch = {'train': batch_train, 'test': batch_test}
...
def getLIVEMVQA(cfg, transform_train, transofrm_test, batch_train=1, batch_test=1, train_percent=0.8, idx=0, train=True):
    summary_dir = 'runs/swin_LIVEM_VQA/' + DAY
    writer = SummaryWriter(summary_dir)
    check_path = './checkpoints/' + 'LIVEM_VQA/'
    logger = getlogger(path='LIVEM_VQA')
    ...
    video_path = '/home1/server823-2/database/2D-Video/LIVE_Mobile_Video_Quality_Database'
    subj_dataset = './database/VQA/LIVE-M/live_mobile_subj_score.json'
    ...
    if dataset_test == 'LIVE_IQA':
        ...
        test_set = \
            getCSIQVQA(config, transform_train, transform_test, batch_train=batch_test,
                       batch_test=batch_test, train_percent=config.TRAIN.PERCENT, idx=idx, train=False)
    elif dataset_test == 'CVD_VQA':
        test_set = \
            getCVDVQA(config, transform_train, transform_test, batch_train=batch_test,
                      batch_test=batch_test, train_percent=config.TRAIN.PERCENT, idx=idx, train=False)
    elif dataset_test == 'LIVEM_VQA':
        test_set = \
            getLIVEMVQA(config, transform_train, transform_test, batch_train=batch_test,
                        batch_test=batch_test, train_percent=config.TRAIN.PERCENT, idx=idx, train=False)
