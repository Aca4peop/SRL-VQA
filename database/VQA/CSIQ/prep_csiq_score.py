...
def make_score_file():
    ...
    train_ratio = 0.8
    index_list = list(range(len(seqs)))
    random_list = list(range(len(seqs)))
    random.shuffle(random_list)
    train_num = int(train_ratio*len(seqs))


    for i, r in zip(index_list, random_list):
        clip = seqs[r]
        mos = score[r]
        mos = mos[0]
        # clip_info = re.split('_', clip)
        clip_name = clip.split('_')
        # clip = '' + clip_name + '{dst:02d}'.format(dst=dst) + '_' + clip_info[-1]
        # clip = '' + clip+'.yuv'
        ref = '_'.join(clip_name[:2]) + '_ref.yuv'
        # ref = '' + clip_name + '_ref' + '.yuv'
        fps = framerate[ref]
    ...
    with open('csiq_subj_score_nr.json', 'w') as f:
