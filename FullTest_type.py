...
def testLoop(config, test_set, model, device, logger, epoch, disNum):
    ...
    with torch.no_grad():
        for (img, label, disType) in tqdm(test_set):
            ...
            for i in range(B):
                disListLabel[disType[i]].append(float(label[i][0].cpu().numpy()))
            ...
            epoch_dis.extend(disType.cpu().numpy().reshape(-1))
            

        mdict = {'pred' : epoch_pred, 'label' : epoch_label, 'dis_type' : epoch_dis}
